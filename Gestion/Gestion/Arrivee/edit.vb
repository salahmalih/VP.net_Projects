...
Public Class edit
    ...
    Sub combo()
        Try
            co.Open()
            cmd.Connection = co
            cmd.CommandText = "Select NUmcommande from Arrivee order by NUmcommande "
    ...
    Sub pde(a As String)
        ...
        Try
            co.Open()
            Dim cmd1 As New OleDbCommand("SELECT fil FROM Arrivee WHERE NUmcommande = " & a & "", co)
            pdfFile = DirectCast(cmd1.ExecuteScalar(), Byte())
        Catch ex As Exception
            MsgBox("Il n'y a pas de fichier dans les données sous-jacentes", MsgBoxStyle.Critical, "Arrivee")
        End Try
        co.Close()
        ...
        If pdfFile IsNot Nothing Then
            Try
                co.Open()
                Dim cmd2 As New OleDbCommand("SELECT Pfile FROM Arrivee WHERE NUmcommande = " & a & "", co)
    ...
    Private Sub btnajouter_Click(sender As Object, e As EventArgs) Handles btnupdate.Click
        ...
        Else
            Try
                co.Open()
                cmd.Connection = co
                cmd.CommandText = "Update Arrivee set [expediteur]='" & txtsender.Text & "', [tresenvoye]='" & txtobjetsende.Text & "',[Datese]='" & txtdate.Value & "', [Pfile]='" & lblpath.Text & "' , [fil]=@file ,CIN='" & admin.cin & "'  where [NUmcommande]=" & txtnumco.Text & ""
